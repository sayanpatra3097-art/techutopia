...
// 1 Core Member
const coreMember = [
    { name: 'Shoury Bansal', initials: 'SB', image: '', role: 'Core Developer' },
];

// 4 Coordinators
const coordinators = [
    { name: 'Prateek Saxena', initials: 'PS', image: '/team/coordinator1.jpg', role: 'Coordinator' },
    { name: 'Ishit Agrawal', initials: 'IA', image: '', role: 'Coordinator' },
    { name: 'Kartik Gupta', initials: 'KG', image: '', role: 'Coordinator' },
    { name: 'Yash Bansal', initials: 'YB', image: '', role: 'Coordinator' },
];
...
function TeamSection({ title, members, isMainHeading = false, isCoreSection = false }: { title: string; members: Member[]; isMainHeading?: boolean; isCoreSection?: boolean }) {
    ...
    return (
        <div className="mb-20 flex flex-col items-center">
            {title && (
                <div className="text-center mb-16 relative">
                    ...
                    <div className="relative inline-block">
                        ...
                        <h2 className={`relative font-black tracking-tight bg-gradient-to-r from-purple-300 via-pink-300 to-indigo-300 bg-clip-text text-transparent drop-shadow-[0_4px_24px_rgba(139,92,246,0.6)] ${isMainHeading ? 'text-6xl sm:text-7xl md:text-8xl' : 'text-4xl sm:text-5xl md:text-6xl'
                            }`}>
                            {title}
                        </h2>
                    </div>
            ...
            <div className={`grid gap-8 sm:gap-10 ${isCoreSection ? 'grid-cols-1' : 'grid-cols-1 sm:grid-cols-2 lg:grid-cols-4'
                }`}>
                {members.map((m, index) => (
                    <figure
                        key={m.name}
                    ...
                    >
                        ...
                        >
                            ...
                            <div className="relative w-full aspect-[3/4] min-h-[450px]">
                                {m.image ? (
                                    <>
                                        ...
                                        <img
                                            src={m.image}
                                            alt={m.name}
                                            className="absolute inset-0 w-full h-full object-contain opacity-95 drop-shadow-[0_10px_30px_rgba(0,0,0,0.55)] transition-transform duration-700 will-change-transform p-0 sm:p-0.5 scale-[1.28] group-hover:scale-[1.34]"
                                            onError={(e) => {
...
export default function DevelopedByPage() {
    return (
        <div className="min-h-screen text-white overflow-hidden relative font-sans">
            ...
            {/* Background overlay from assets */}
            <div className="fixed inset-0 -z-10 bg-[#0c0a09]">
                <Image
                    ...
                    priority
                />
                <div className="absolute inset-0 bg-gradient-to-b from-[#0c0a09]/50 via-transparent to-[#0c0a09]/80" />
                ...
                <div className="absolute inset-0 bg-[radial-gradient(circle_at_20%_90%,rgba(59,130,246,0.1),transparent_30%)]" />
            </div>

            {/* Main content with enhanced styling */}
            <section className="px-4 pt-44 pb-24 relative z-10 max-w-7xl mx-auto">
                <div className="text-center mb-16">
                    <h1 className="text-5xl md:text-6xl font-black font-[Cinzel] tracking-widest text-[#d4af37] uppercase mb-4">
                        Developed By
                    </h1>
                    <p className="text-stone-400 italic font-serif">The creative minds behind this experience.</p>
                </div>
                ...
                <TeamSection title="Coordinators" members={coordinators} isMainHeading={false} isCoreSection={false} />
            </section>
        </div>
    );
}
